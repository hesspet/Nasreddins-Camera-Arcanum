...
/// <summary>
/// Ermittelt das Sternzeichen für ein Datum und mappt es auf die
/// Buchstaben A–L des KR-Astro-3-Fonts.
/// </summary>
public static class SternzeichenHelper
{
    ...
    private static bool IstImBereich(DateTime datum, int startMonat, int startTag, int endeMonat, int endeTag)
    {
        ...
        if (startMonat <= endeMonat)
        {
            ...
            if (monat < startMonat || monat > endeMonat) return false;
            if (monat == startMonat && tag < startTag) return false;
            if (monat == endeMonat && tag > endeTag) return false;
            return true;
        }
        else
        {
            // Jahresübergreifend (z.B. Dezember–Januar, Steinbock)
            if (monat == startMonat && tag >= startTag) return true;
            if (monat == endeMonat && tag <= endeTag) return true;
            return false;
        }
    }
}
